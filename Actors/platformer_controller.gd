...
@export var input_crouch : String = "move_down"
...
#@export var sprite_animation : AnimatedSprite2D = $AnimatedSprite2D
...
func _input(_event):
	...
	if Input.is_action_just_pressed(input_jump):
		...
		if not can_hold_jump and can_ground_jump():
			jump()
	...
	if Input.is_action_pressed(input_crouch):
		if not is_moving or not is_feet_on_ground():
...
func set_crouch_state(crouched: bool):
	if crouched:
		hit_box.shape.size.y = 19
		hit_box.position.y = -9.5
		is_crouched = true
	
	if not crouched:
		hit_box.shape.size.y = 24
		hit_box.position.y = -12
		is_crouched = false
